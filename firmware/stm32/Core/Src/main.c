...
/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
Timing_HandleTypeDef_t htiming;
MEAS_HandleTypeDef_t hmeas;
VCO_HandleTypeDef_t hvco;
STDWN_HandleTypeDef_t hstdwn;
BUZ_HandleTypeDef_t hbuz;
/* USER CODE END PM */
...
/* USER CODE BEGIN PV */
osThreadId_t SAMPLING_TaskHandle;
const osThreadAttr_t SAMPLING_Task_attributes = { .name = "SAMPLING_Task", .stack_size =
		8192, .priority = (osPriority_t) osPriorityHigh, };
/* USER CODE END PV */
...
int main(void) {
	...
	/* USER CODE BEGIN 2 */
	Timing_Init(&htiming, &htim2, &htim8);
	VCO_Init(&hvco, &hdac, 256, DAC_TRIGGER_T8_TRGO);
	meas_init(&hmeas, &hadc1, &hadc2);
	//buzzer_init(&htim10, &hbuz);
	...
	SAMPLING_TaskHandle = osThreadNew(Sampling_Task, NULL, &SAMPLING_Task_attributes);
...
static void MX_GPIO_Init(void) {
	...
	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOC, SIG_SEL_Pin | GPIO_PIN_2, GPIO_PIN_RESET);

	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOD, ESP_EN_Pin | GPIO_PIN_12 | GPIO_PIN_13,
			GPIO_PIN_RESET);
	...
	/*Configure GPIO pin : SIG_SEL_Pin */
	GPIO_InitStruct.Pin = SIG_SEL_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(SIG_SEL_GPIO_Port, &GPIO_InitStruct);
	...
	/*Configure GPIO pin : EN_PCB_OUT_Pin */
	GPIO_InitStruct.Pin = EN_PCB_OUT_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(EN_PCB_OUT_GPIO_Port, &GPIO_InitStruct);

	/*Configure GPIO pin : EN_PCB_IN_Pin */
	GPIO_InitStruct.Pin = EN_PCB_IN_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(EN_PCB_IN_GPIO_Port, &GPIO_InitStruct);

	/* USER CODE BEGIN MX_GPIO_Init_2 */
	// TODO gut an diesem ORT?
	// NVIC für EXTI Line 3 aktivieren
...
void dwt_enable_cycle_counter(void)
{
    ...
    // Reset the counter before enabling
    DWT->CYCCNT = 0;

    // Enable the cycle counter if not already on
...
Error_t start_sequence(){

	exec(VCO_start(&hvco));
	dwt_enable_cycle_counter();
	exec(Timing_start_seq(&htiming));
	exec(meas_start_Sampling(&hmeas));

	//uint32_t stop = DWT->CYCCNT;
	//volatile uint32_t delay = cycles2-cycles;
	RETURN_OK();
}
Error_t stop_sequence(){
	hmeas.state = MEAS_WAITING;
	exec(meas_stop_Sampling(&hmeas));
	exec(Timing_stop_seq(&htiming));
	exec(VCO_stop(&hvco));
...
void Sampling_Task(void *argument) {
	/* USER CODE BEGIN 5 */
	logMsg(LOG_INFO, "hello");
	meas_setTaskHandle(SAMPLING_TaskHandle);
	exec(start_sequence());
	...
	for (;;) {
		...
		meas_task(&hmeas);
		if(hmeas.data.fft_buf_index == 3){
			float32_t *data_to_send = &hmeas.data.fft_buf[0]; // first frame
			HAL_UART_Transmit_DMA(&huart1, (uint8_t *)data_to_send, sizeof(float32_t) * MEAS_SAMPLE_BUFFER_LENGTH/2);
		}
	}
	/* USER CODE END 5 */
}
